...
export default function AddPaymentDialog({ open, onOpenChange, customerId, onPaymentAdded }: AddPaymentDialogProps) {
  ...
  const onSubmit: SubmitHandler<PaymentFormData> = async (data) => {
    ...
    try {
      ...
      reset({ date: new Date().toISOString().split('T')[0], paymentMode: 'Cash' });
      onOpenChange(false);
    } catch (error) {
      toast({
        title: 'Error',
